...
for batch in [128]:
    ...
    for i in tqdm(range(10)):
        ...
        """
        #学習途中のパラメータ
        torch.save(net.state_dict(), f"./output/knock79_checkpoints/batch{batch}/model{i}.pth")
        #最適化アルゴリズムの内部状態
        torch.save(optimizer.state_dict(), f"./output/knock79_checkpoints/batch{batch}/optime{i}.pth")
        """
    print(f'----------batch={batch}----------')
...
plt.xlabel("epoch")
plt.ylabel("loss")
plt.savefig("./output/knock79_loss.png")
fig2 = plt.figure()
...
plt.xlabel("epoch")
plt.ylabel("accuracy")
plt.savefig("./output/knock79_accuracy.png")
